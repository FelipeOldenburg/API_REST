...
const list = async (usuario) => {
  const sql = `SELECT c.id, c.titulo, c.descricao, c.status, c.criado_em, c.atualizado_em,
               ...
               LEFT JOIN comentarios_chamado co ON co.chamado_id = c.id
...
const findById = async (id, usuario) => {
  ...
  const [comentarios] = await db.execute(
    `SELECT co.id, co.mensagem, co.criado_em, u.nome AS autor, u.papel
     FROM comentarios_chamado co JOIN usuarios u ON u.id = co.autor_id
...
const updateStatus = async (id, status) => {
  const [result] = await db.execute('UPDATE chamados SET status = ? WHERE id = ?', [status, id]);
  return result.affectedRows > 0;
};

const addComentario = async (id, mensagem, autorId) => {
  const [result] = await db.execute(
    'INSERT INTO comentarios_chamado (chamado_id, autor_id, mensagem) SELECT id, ?, ? FROM chamados WHERE id = ?',
    [autorId, mensagem, id]
  );
